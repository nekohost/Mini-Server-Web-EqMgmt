...
import { createEvent, emptyState, eventAlreadyRecorded, projectEvents, renderEventBlock } from '../.agent-governance/tooling/conversation-recorder/core.mjs';
import { parseCodexTranscript } from '../.agent-governance/tooling/conversation-recorder/codex.mjs';
import { collectAntigravityEvents } from '../.agent-governance/tooling/conversation-recorder/antigravity.mjs';

const HERE = path.dirname(fileURLToPath(import.meta.url));
const FIXTURES = path.resolve(process.env.EQM_FIXTURE_ROOT || path.join(HERE, '..', '.agent-governance', 'tooling', 'fixtures', 'conversation-recorder'));
const FIXTURE_WORKSPACE = 'C:\\fixture\\project';

test('legacy recipient matching is bidirectional but never matches another AI', () => {
  const event = createEvent({ provider: 'codex', threadId: 'compat', sourceEventId: '1', occurredAt: '2026-09-09T01:00:00Z', actor: 'user', channel: 'user', speaker: '사용자', content: 'same text' });
  assert.equal(eventAlreadyRecorded(`## 사용자 → Codex ${event.time.header}\n\nsame text\n`, event).matched, true);
  assert.equal(eventAlreadyRecorded(`## 사용자 → Gemini ${event.time.header}\n\nsame text\n`, event).matched, false);
  assert.equal(createEvent({ ...event, speaker: '사용자 → Codex' }).eventId, event.eventId);
});
