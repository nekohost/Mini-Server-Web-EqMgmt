...
export function eventAlreadyRecorded(markdown, event) {
  ...
  // 사용자 발언의 경우 '사용자 → <AI>' 신규 규격과 기존 '사용자' 단독 표기를 모두 후보로 검사한다.
  const candidateHeaders = [`## ${event.speaker} ${event.time.header}\n\n`];
  if (event.actor === 'user' && event.speaker.startsWith('사용자 → ')) {
    candidateHeaders.push(`## 사용자 ${event.time.header}\n\n`);
  }
...
export function insertEventBlock(markdown, event) {
  ...
  if (next) {
    ...
    if (markerIndex >= 0 && prefix.slice(markerIndex).trim().endsWith('-->')) insertAt = markerIndex + 1;
  }
  // 블록 앞뒤 빈 줄을 정확히 정리해 삽입한다.
  const before = current.slice(0, insertAt).replace(/\n*$/, '');
  const after = current.slice(insertAt).replace(/^\n*/, '');
  const block = renderEventBlock(event).trimEnd();
  const leading = before ? `${before}\n\n` : '';
  const trailing = after ? `\n\n${after}` : '\n';
  return `${leading}${block}${trailing}`;
}

// 텍스트를 UTF-8 바이트 경계를 깨지 않고 지정 크기 이하 조각으로 나눈다.
export function splitUtf8Text(text, limitBytes = COMPANION_LIMIT_BYTES) {
  // 조각 배열을 준비한다.
  const chunks = [];
  // 현재 조각의 버퍼를 문자열로 누적한다.
  let current = '';
  let currentBytes = 0;
  // 문자열의 코드 포인트 단위로 순회하여 surrogate pair를 보존한다.
