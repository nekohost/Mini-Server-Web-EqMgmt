...
// [의존성 관계] candidate_core.mjs와 <appDataDir>/brain/<conversation-id>/.system_generated/logs/transcript.jsonl 형식에 의존한다.
...
import { createEvent, normalizeNewlines, normalizePathForComparison, parseJsonLines, sourceFingerprint } from './candidate_core.mjs';
...
function conversationEvents(info, { isSubagent = false, parentThreadId = '' } = {}) {
  ...
  for (const row of info.rows) {
    ...
    if (item?.source === 'USER_EXPLICIT' && item?.type === 'USER_INPUT' && item?.status === 'DONE') {
      ...
      // 직접 사용자 입력은 수신 대상을 명시한 '사용자 → Gemini'로 기록한다.
