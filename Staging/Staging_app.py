...
# Staging 전용 템플릿 로더 오버라이딩 (Rule 7-3-2 격리 원칙 준수)
import jinja2
app.jinja_loader = jinja2.ChoiceLoader([
    jinja2.FileSystemLoader('Staging'),
...
@app.route('/staging/dashboard')
@login_required
def staging_dashboard_page():
    """
    [역할] Staging 격리 검증용 통계 대시보드 화면을 렌더링합니다.
    [의존성 관계] Staging/Staging_dashboard.html 템플릿 및 @login_required
    [변경 시 영향도] /staging/dashboard 접속 시 Staging 대시보드 페이지 반환
    """
    if not check_menu_permission('dashboard'):
        return "<script>alert('접근 권한이 없습니다.'); location.href='/portal';</script>"
