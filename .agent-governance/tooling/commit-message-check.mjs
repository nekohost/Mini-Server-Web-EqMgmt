...
const message = (args[0] === '--file' ? fs.readFileSync(args[1], 'utf8') : args.join(' ')).replace(/\r\n/g, '\n').trim();
...
if (!message) { // 제목 누락을 먼저 거부합니다.
...
} else if (/^Merge (?:branch |remote-tracking branch |pull request |tag )\S.+$/.test(subject) || (/^Revert ".+"$/.test(subject) && /\bThis reverts commit [a-f0-9]{7,40}\./.test(bodyLines.join('\n')))) { // 표준 자동 메시지 형태만 예외입니다.
  console.log('COMMIT-MESSAGE: AUTO-GENERATED-EXCEPTION'); // 수동 한국어 규칙과 구분합니다.
} else { // 일반 AI 작성 commit을 검사합니다.
  const match = subject.match(/^([a-z]+)(?:\([^)\r\n]+\))?!?:[ \t]+(.+)$/); // Conventional Commit 기본 형식입니다.
  ...
  } else if (bodyLines.some(line => line.trim()) && !/[가-힣]/.test(bodyLines.join('\n').replace(/^(?:Signed-off-by|Co-authored-by):.*$/gm, ''))) {
    console.error('COMMIT-MESSAGE: Korean body required');
    process.exitCode = 1;
