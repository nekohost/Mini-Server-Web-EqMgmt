...
const wrapperPath = fileURLToPath(new URL('./git-readonly.mjs', import.meta.url));
...
test('임시 저장소의 status probe는 index 바이트와 mtime을 변경하지 않는다', () => {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), 'git-readonly-test-'));
  try {
    ...
    const result = spawnSync(process.execPath, [wrapperPath, '--', 'status', '--short'], {
      cwd: workspace,
      encoding: 'utf8',
      windowsHide: true
    });
