...
with open(log_path, 'r', encoding='utf-8') as f, open('scratch_out.txt', 'w', encoding='utf-8') as out:
    for line in f:
        ...
        if data.get('type') == 'USER_INPUT' and '<USER_REQUEST>' in data.get('content', ''):
            ...
            out.write(f"USER ({data.get('created_at')}): {req}\n")
        elif data.get('type') == 'PLANNER_RESPONSE' and not data.get('tool_calls') and data.get('content'):
            out.write(f"AI ({data.get('created_at')}): {data.get('content')}\n")
