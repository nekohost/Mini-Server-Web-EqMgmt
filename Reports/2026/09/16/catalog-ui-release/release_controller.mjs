// [역할] 승인된 백업 서버 배포 제어. [의존성 관계] 기존 Windows SSH agent. [변경 시 영향도] 비밀을 출력/전달하지 않습니다.
...
const digest=bytes=>createHash('sha256').update(bytes.toString('utf8').replaceAll('\r\n','\n'),'utf8').digest('hex'); // 현재 운영 자산은 텍스트입니다.
const files=Object.fromEntries(all.map(name=>[name,{previous:oldNames.has(name)?digest(git('show',previous+':'+name)):null,target:digest(git('show',target+':'+name))}])); // 내용은 전송하지 않고 hash만 전송합니다.
...
const payload={target,files,server_head:'5ab042a7cb1a4acd25af64732c6a1f64b644db94',pid:91637,apply:process.argv[2]==='apply'}; // 확인된 기존 상태가 달라지면 서버가 거부합니다.
...
fs.writeFileSync(path.join(here,(payload.apply?'deployment':'preflight')+'-result.json'),JSON.stringify(report,null,2),{flag:'wx'}); // 결과를 덮어쓰지 않습니다.
