...
const baselines = JSON.parse(readFileSync(new URL('portrait-baselines.json', import.meta.url), 'utf8'));
...
if (!['check', 'approved'].includes(process.env.EQM_DEPLOY_PORTRAIT)) throw new Error('Set EQM_DEPLOY_PORTRAIT=check or approved for this release.');
const data = {apply: process.env.EQM_DEPLOY_PORTRAIT === 'approved', head: gitRead(['rev-parse', 'HEAD']).trim(), pid: 78371, files: {}};
...
const script = readFileSync(new URL('deploy-portrait.py', import.meta.url), 'utf8');
