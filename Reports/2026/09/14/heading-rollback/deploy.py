...
FILES = {"static/css/layout.css","templates/index.html","templates/master_management.html","templates/access_logs.html","templates/access_logs_error_ips.html","templates/lineup_management.html","templates/permissions.html","templates/mypage.html","templates/users_management.html"}
...
for name, entry in data["files"].items():
    ...
    if entry["baseline"] is None:
        ...
        old = path.read_bytes()
        assert sha(old) == entry["baseline"], "baseline mismatch: " + name
...
# Selective rollback proof, not a whole-release/DB restore.
previous_release = RELEASES / "ui-responsive-shell-20260914T064237Z"
previous = json.loads((previous_release / "result.json").read_text())
preserved = {n: v["after"] for n, v in previous["files"].items() if n not in FILES}
assert len(preserved) == 5
def preserved_match():
    return {n: sha((ROOT / n).read_bytes()) == value for n, value in preserved.items()}
assert all(preserved_match().values())
heading_only = FILES - {"static/css/layout.css", "templates/users_management.html"}
for name in heading_only:
    assert normalized(candidates[name]) == normalized((previous_release / "before" / name).read_bytes()), name
old_css = normalized(originals["static/css/layout.css"][0]).decode()
start = old_css.index("/* Page titles and actions share")
end = old_css.index("/* Compact two-row side groups", start)
expected_css = old_css[:start] + "[data-selection-action]:disabled { opacity: .45; cursor: not-allowed; box-shadow: none; }\n\n" + old_css[end:]
expected_css = expected_css.replace(".equipment-page-actions button { white-space: nowrap; }", ".equipment-page-actions button, .account-profile-heading > * { white-space: nowrap; }")
assert normalized(candidates["static/css/layout.css"]).decode() == expected_css, "CSS rollback scope mismatch"
expected_users = normalized(originals["templates/users_management.html"][0]).decode().replace('app-page-heading app-heading-split mb-4', 'app-page-heading mb-4 flex flex-col md:flex-row md:items-center justify-between gap-4').replace('class="app-heading-actions"', 'class="flex flex-wrap items-center gap-2"')
assert normalized(candidates["templates/users_management.html"]).decode() == expected_users, "Selection behavior must remain unchanged"

database = ROOT / "equipment.db"
before = db_state(database)
assert before["integrity"] == "ok" and before["foreign_key_violations"] == 0
if not data["apply"]:
    print(json.dumps({"status": "ready", "baseline_head": head, "pid": old_pid, "file_count": len(FILES), "preserved_files": preserved_match(), "rollback_scope_verified": True, "db": before}))
    sys.exit(0)
release = RELEASES / ("ui-heading-rollback-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ"))
...
try:
    ...
    assert all(preserved_match().values()), "preserved UI changed"
    assert after == before, "business counts or integrity changed"
    evidence.update(status="deployed", new_pid=new_process.pid, db_after=after, health=200, preserved_files=preserved_match(), rollback_scope_verified=True)
