...
FILES = {"static/css/roadmap.css", "static/css/layout.css", "templates/access_logs.html",
...
def write_atomic(path, data, mode):
    ...
    temporary = path.with_name(path.name + ".edge-release-" + str(os.getpid()))
...
assert not subprocess.check_output(["git", "status", "--porcelain", "--untracked-files=no"], cwd=ROOT, env=git_env), "remote tracked edits"
old_pid = data["pid"]
...
for name, entry in data["files"].items():
    ...
    if entry["baseline"] is None:
        assert name == "static/css/layout.css" and not path.exists()
        originals[name] = None
    else:
        old = path.read_bytes()
...
if not data["apply"]:
    ...
    sys.exit(0)
release = RELEASES / ("edge-layout-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ"))
