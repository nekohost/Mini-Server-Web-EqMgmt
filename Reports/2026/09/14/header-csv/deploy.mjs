...
if (!['check', 'approved'].includes(process.env.EQM_DEPLOY_HEADER_CSV)) throw new Error('Set EQM_DEPLOY_HEADER_CSV=check or approved for this release.');
const data = {apply: process.env.EQM_DEPLOY_HEADER_CSV === 'approved', head: gitRead(['rev-parse', 'HEAD']).trim(), pid: 80368, files: {}};
