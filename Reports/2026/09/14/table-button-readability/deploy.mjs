...
if (!['check', 'approved'].includes(process.env.EQM_DEPLOY_READABILITY)) throw new Error('Set EQM_DEPLOY_READABILITY=check or approved for this release.');
const data = {apply: process.env.EQM_DEPLOY_READABILITY === 'approved', head: gitRead(['rev-parse', 'HEAD']).trim(), pid: 82278, files: {}};
