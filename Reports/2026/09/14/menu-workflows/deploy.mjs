...
const record = JSON.parse(readFileSync(new URL('baselines.json', import.meta.url), 'utf8'));
...
if (!['check', 'approved'].includes(process.env.EQM_DEPLOY_MENU_WORKFLOWS)) throw new Error('Set EQM_DEPLOY_MENU_WORKFLOWS=check or approved for this release.');
const data = {apply: process.env.EQM_DEPLOY_MENU_WORKFLOWS === 'approved', head: gitRead(['rev-parse', 'HEAD']).trim(), pid: record.pid, files: {}};
