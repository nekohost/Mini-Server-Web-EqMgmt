...
FILES = {"templates/register.html","templates/reset_password.html"}
...
# Label-only invariant: form controls, event handlers and authentication scripts stay byte-identical.
    expected = old.replace(b" (Staging)", b"").replace(" (Staging 검증)".encode(), b"")
    assert normalized(candidate) == normalized(expected), "not a label-only change: " + name
# Full candidate template syntax before stopping the live app; no app import.
import jinja2
...
if not data["apply"]:
    ...
    sys.exit(0)
release = RELEASES / ("ui-menu-workflows-auth-labels-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ"))
